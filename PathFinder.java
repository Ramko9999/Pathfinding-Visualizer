...
public class PathFinder {
    ...
    public static void main(String[] args) {
        PathFinder pathFinder =  new PathFinder(new int[24][24], 9, 10, 1, 1);
    ...
    public void Greedy(){
        ...
        displayArray(board);
        MinList costList = new MinList();
        costList.appendG(new Node(new LocNode(startRow, startCol, 0), endRow, endCol, 0)); //append the first node or starting point to our list
        Node costHeader = costList.head;




        //condition checks whether we reached end point
        while(!(costHeader.element.row == endRow && costHeader.element.col == endCol)){

            displayArray(board);

            //gets a list of possible places we can move from our current position
            ArrayList<Node> possibleNodes = generateNewNodesForAStar(costHeader,  endRow, endCol, board);


            costList.head = costList.head.next; //pop out the node

            for(Node n : possibleNodes){
                costList.appendG(n); //appends the places in an order maintained way

            }
            costList.print();
            costHeader = costList.head; //keep going
            costList._size--;
            if(costList._size == 1){
                costList.print();
                System.out.println(costList._size);
        ...
        while(costHeader.element != null){

            System.out.println(costHeader.element.row + " , " + costHeader.element.col);
            board[costHeader.element.row][costHeader.element.col] = 4;
            counter++;
            costHeader.element = costHeader.element.previous;
        }
        displayArray(board);
...
//if our MaxList is empty
        ...
        //traverse till we find a place to put our nextNode
            ...
            while (current != null) {
                ...
                if (nextNode.cost < current.cost) {
                    ...
                    System.out.println(current.element.row + "," + current.element.col);
    ...
    class Node {
        ...
        void calcCost(int sRow, int sCol) {
            int row = element.row;
            int col = element.col;

            cost = 10 * Math.sqrt(Math.pow((double) (row - sRow), 2.0) + Math.pow((double) (col - sCol), 2.0));
